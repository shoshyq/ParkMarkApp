...
namespace BL
{
    public class MainBL
    {
        ...
        {
            var yesterday = DateTime.Today.AddDays(-3);
        ...
        {
            Entities.SearchResult sr = DAL.Convert.SearchResultConvert.ConvertSearchResultListToEntity(DbHandler.GetAll<DAL.SearchResult>()).First(s => s.Code == srCode);
            Entities.ParkingSpot pspot = DAL.Convert.ParkSpotConvert.ConvertParkingSpotsListToEntity(DbHandler.GetAll<DAL.ParkingSpot>()).First(y => y.Code == sr.ResultPSCode);
            Entities.ParkingSpotSearch psearch = DAL.Convert.SearchConvert.ConvertSearchesListToEntity(DbHandler.GetAll<DAL.ParkingSpotSearch>()).First(y => y.Code == sr.SearchCode);
            Entities.User spOwner = DAL.Convert.UserConvert.ConvertUsersListToEntity((IEnumerable<DAL.User>)DbHandler.GetAll<DAL.User>()).First(u => u.Code == pspot.UserCode);
            Entities.User spRenter = DAL.Convert.UserConvert.ConvertUsersListToEntity((IEnumerable<DAL.User>)DbHandler.GetAll<DAL.User>()).First(u => u.Code == psearch.UserId);
            ...
            string body = "";
            body += string.Format("User {0} is going to rent Your parking spot on {1} <br />", spRenter.Username, pspot.FullAddress);
        ...
        {
            if (DAL.Convert.SearchResultConvert.ConvertSearchResultListToEntity(DbHandler.GetAll<DAL.SearchResult>()).Any(r => r.Usercode == usercode))
            {             
                var todelete = DAL.Convert.SearchResultConvert.ConvertSearchResultListToEntity(DbHandler.GetAll<DAL.SearchResult>()).Where(r => r.Usercode == usercode).ToList();

                foreach (var item in todelete)
                {
                    if (DAL.Convert.SearchConvert.ConvertSearchesListToEntity(DbHandler.GetAll<DAL.ParkingSpotSearch>()).First(y => y.Code == item.SearchCode).SearchDate < DateTime.Now.AddDays(-7))
                        DbHandler.DeleteSet<SearchResult>(item);
                }
            }
            if (DAL.Convert.SearchResultConvert.ConvertSearchResultListToEntity(DbHandler.GetAll<DAL.SearchResult>()).Any(r => r.Usercode == usercode))
